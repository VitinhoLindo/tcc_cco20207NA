<template>
  <div id="app" v-on:contextmenu="contextMenu">
    <app-loading v-if="loading.on" v-bind:loading="loading.type" />

    <app-acesso v-if="!auth" v-bind:functions="functions" v-bind:shared="shared" />
    ...
    <app-menu v-bind:functions="functions" v-bind:shared="shared" v-bind:auth="auth" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async maximize(
      ...
      if (offset) {
        this.setOffSet(div, offset.height, offset.width);
        ...
        this.setOffSet(div, innerHeight - offsetHeight, innerWidth);
      }
    },
    is_numeric(value) {
    ...
    eventPromise(opt = { eventName: "", data: {}, awaitResponse: false }) {
      return new Promise((resolve, reject) => {
        ...
        if (opt.data)
          this.emmiter(opt.eventName, opt.data, (err, result) => {
            if (err) return reject(err);
            else return resolve(result);
          });
        else
          this.emmiter(opt.eventName, (err, result) => {
            if (err) return reject(err);
            else return resolve(result);
          });
      });
    ...
    getHeader(headers) {
      ...
      for (let key in headers) _defaultHeader[key] = headers[key];

      if (this.auth) _defaultHeader["Autentication"] = `Bearer ${this.auth}`;
