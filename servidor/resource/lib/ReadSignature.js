...
class FileSignature {
  ...
  static async identify(path) {
    ...
    for (i = signatures.length - 1; i >= 0; i--) {
      let _buf = signatures[i];
      if (this.bufferIsEqual(buf.slice(0, signatures[i].byteSeq.length), _buf.byteSeq)) {
        return {
          extension: _buf.extension,
          description: _buf.description || '',
          mime: _buf.mimeType.mime || 'application/octet-stream'
        }
      }
    }
